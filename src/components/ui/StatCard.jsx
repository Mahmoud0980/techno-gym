...
function MiniSpark({ tone = "yellow" }) {
  ...
  return (
    <div className={`relative h-7 w-16 overflow-hidden border-b-2 ${color}`}>
      <span className="absolute bottom-1 left-0 h-4 w-4 rotate-45 border-t-2 border-current text-inherit opacity-70" />
      <span className="absolute bottom-2 left-4 h-5 w-5 rotate-45 border-t-2 border-current text-inherit opacity-90" />
      <span className="absolute bottom-1 left-9 h-4 w-5 rotate-45 border-t-2 border-current text-inherit opacity-80" />
    </div>
  );
}

export default function StatCard({ title, value, change, helper, tone = "yellow", negative = false, compact = false }) {
